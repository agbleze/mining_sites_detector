...
def evaluate_model_multi_seed(checkpoint_path, batch_size=64, device="cuda",
...
def evaluate_multi_seed(checkpoint_path, batch_size=64, device="cuda",
    ...
    for seed_key, model_obj in run_history.items():
        ...
        print(f" ===> Seed {seed_key}: Test Accuracy: {test_accuracy:.5f}")
    ...
    std_acc = np.std(test_acc, ddof=1) * 100 if num_runs > 1 else 0.0
    ...
    print(f"MANUSCRIPT METRIC  : {mean_acc:.4f} (± 95% CI: {ci_acc:.4f})  [Std Dev: {std_acc:.4f}]")
    print("="*80 + "\n")
...
# %%
checkpoint_path = "/home/lin/codebase/mining_sites_detector/src/mining_sites_detector/layers_[32]_stride_2_pool_None_use_bn_True_decoder_use_bn_True_batch_size_32_seed_9999.pkl"
evaluate_multi_seed(checkpoint_path, batch_size=32, device="cuda")
# %%
...
print(f"[INFO] Found {len(full_checkpoint_files)} full checkpoint files with seed 9999:")
